...
fn main() -> Result<()> {
    ...
    // for (_, anim) in qual.anims.iter_mut() {
    //     match anim {
    //         Some(BoneAnim::Rotation(p)) => { p.to_pose();},
    //         Some(BoneAnim::Position(p)) => { p.to_pose();},
    //         Some(BoneAnim::RotationIK { rotation, target }) => { rotation.to_pose(); target.to_pose(); }
    //         Some(BoneAnim::ArmIK { target, rotation }) => {target.to_pose(); rotation.to_pose()},
    //         Some(BoneAnim::PositionIKRotation { position, target }) => { position.to_pose(); target.to_pose() },
    //         Some(BoneAnim::PositionRotation { position, rotation }) => { position.to_pose();},
    //         _ => continue
    //     }
    //     // println!("reset bone");
    // }
